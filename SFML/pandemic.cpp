...
life::Pandemic::Pandemic(int &N)
    : pan_side{N}, pan_grid(N * N, life::Person::Susceptible) {
...
bool life::operator!=(life::Pandemic const &left, life::Pandemic const &right) {
  return left.pan_grid != right.pan_grid;
}
...
life::Person &life::Pandemic::Person_cell(
...
int life::Pandemic::side() const { return pan_side; }
...
int life::Pandemic::infected_neighbours(Pandemic &pandemic, int r, int c) {
  int contacts = 0;
  for (int i : {-1, 0, 1}) {   // vado su e giù nelle righe
    for (int j : {-1, 0, 1}) { // vado a destra e a sinistra nelle colonne
      if (pandemic.Person_cell(r + i, c + j) == Person::Infected)
        contacts++;
    }
  }
...
life::Pandemic life::Pandemic::evolve(
    Pandemic &current, float &beta, float &gamma, float &alfa1, float &alfa2,
    float &omega, float &s, float &i, float &r, float &d, int const &T,
    double const &t_m) // ho messo le probabilità come reference
                       // per alleggerire l'allocazione in memoria
{
  int l = current.side(); // lato della griglia dell'oggetto current
  // TODO: next non esiste come funzione !!!
  life::Pandemic next(l); // uso il costruttore di pandemic che riechiede come
    ...
    for (int c = 0; c <= l; c++) { // loop sulle celle di ogni riga
          ...
          life::Person::Susceptible) // suscettibili
      {
        ...
        if (alfa1 != 0 && 0 < p1 <= (alfa1 / 100)) {
          ...
          r++;
          s--;
        } else if ((alfa1 / 100) < p1 && p1 <= (alfa1 / 100) + b) {
